...
def coffee_drink?(drink_list)
  ...
  # drink_list.any? { |drink| drink.eql?("coffee") || drink.eql?("espresso") }
  # drink_list.include?("coffee") || drink_list.include?("espresso")
  # drink_list.intersect?(["coffee", "espresso"])

  # Do this once
  drink_set = drink_list.to_set 
  # Lookups are now O(1) (constant time) instead of O(n) (linear time)
  drink_set.include?("coffee") || drink_set.include?("espresso")
...
def twenty_first_century_years?(year_list)
  ...
  year_list.all? { |year| 2001 <= year && year < 2001 + 100 }
end
...
def valid_scores?(score_list, perfect_score)
  ...
  # score_list.one? { |key, value| value.equal?(perfect_score) }
